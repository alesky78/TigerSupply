...
public class EnemyTxManager implements TransactionManager {
	...
	public State findNextState(State state, Event event) throws StateMachineException {
		...
		if(stateName.equals(GameResources.STATE_GENERATE_HORDE)){	//GENERA UNA NUOVA ORDA
			if(EventType.equals(GameResources.EVENT_WAIT_KILL)){
				newState = new StateWaitKill();
				newState.setDataModel(dataModel);
				return newState;
			}else if(EventType.equals(GameResources.EVENT_WAIT_TIME)){
				newState = new StateWaitTime();
				newState.setDataModel(dataModel);
				return newState;				
			}else if(EventType.equals(GameResources.EVENT_BOSS_GENERATED)){
		...
		}else if(stateName.equals(GameResources.STATE_WAIT_TIME)){	//ATTENDI TEMPO
		...
		}else if(stateName.equals(GameResources.STATE_WAIT_KILL)){	//ATTENDI MORTE NEMICI
			if(EventType.equals(GameResources.EVENT_WAIT)){
				return state;
			}else if(EventType.equals(GameResources.EVENT_NEW_HORDE)){
				newState = new StateGenerateHorde();
		...
		}else if(stateName.equals(GameResources.STATE_KILL_BOSS)){	//ATTENDI MORTE BOSS
			if(EventType.equals(GameResources.EVENT_WAIT)){
				return state;
			}else if(EventType.equals(GameResources.EVENT_BOSS_KILLED)){
