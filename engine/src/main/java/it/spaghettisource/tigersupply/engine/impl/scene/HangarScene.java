...
import it.spaghettisource.tigersupply.engine.impl.ui.DescriptionListenerHangar;
import it.spaghettisource.tigersupply.engine.impl.ui.HangarDataModel;
import it.spaghettisource.tigersupply.engine.impl.ui.ShipButtonHangar;
import it.spaghettisource.tigersupply.engine.impl.ui.StartButtonHangar;
import it.spaghettisource.tigersupply.engine.impl.ui.WeaponButtonHangar;
...
import it.spaghettisource.tigersupply.engine.impl.weapon.player.Paser;
import it.spaghettisource.tigersupply.engine.impl.weapon.player.DoubleGun;
import it.spaghettisource.tigersupply.engine.impl.weapon.player.SynusoidalGun;
import it.spaghettisource.tigersupply.engine.impl.weapon.player.RocketLauncer;
import it.spaghettisource.tigersupply.engine.impl.weapon.player.Bomb;
...
public class HangarScene extends AbstractGameJPanel {
	...
	public HangarScene(ApplicationContext context,Player player) throws Exception{
		...
		backGround = new StaticBackGroundFitImage(ImageRepositoryManager.getInstance().getSingleImage(StaticResources.BCKGROUND_HANGAR),  pWidth, pHeight);
		...
		//ship button A
		sprite = SpriteFactory.getInstance().createImagePlayerCenterControllerSprite(context.getPeriodMilliseconds(), 800, 2, StaticResources.PLAYER_SHIP_A);
		...
		//ship button B
		sprite = SpriteFactory.getInstance().createImagePlayerCenterControllerSprite(context.getPeriodMilliseconds(), 800, 2, StaticResources.PLAYER_SHIP_B);
