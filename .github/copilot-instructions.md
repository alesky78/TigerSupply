...
`Scene`/`sceneHost`/`sceneHostFactory` contracts, entity/sprite system, collision detection,
...
- Keep framework code in the `engine` module's top-level packages (`entity`, `sprite`, `image`,
  ...
  between them is `engine.control.sceneHostFactory`, implemented by
  `launcher.TigerSupplysceneHostFactory`.
...
- Most services are singletons reached via static `getInstance()`; isolating one for a test
  typically means calling its `init(...)` method first (mirroring how `game.control.TigerSupplysceneHost`
